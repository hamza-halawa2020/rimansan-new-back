...
class PaymobIntegrationService
{
    ...
    public function createPaymentIntention($token, $items, $merchantOrderId, $billingData): ?array
    {
        try {
    ...
    public function credit(string $orderId): array
    {
        ...
        if (!$token) {
            ...
            return ['status' => 500, 'payload' => ['error' => 'Failed to retrieve authentication token']];
        }
        ...
        if (!$paymentIntention) {
            ...
            return ['status' => 500, 'payload' => ['error' => 'Failed to create payment intention']];
        }
        ...
        if (!$clientSecret) {
            ...
            return ['status' => 500, 'payload' => ['error' => 'Client secret not found']];
        }

        $publicKey = env('PAYMOB_PUBLIC_KEY');
        if (!$publicKey) {
            Log::error('Public key not configured');
            return ['status' => 500, 'payload' => ['error' => 'Public key not configured']];
        }
        ...
        return ['status' => 200, 'payload' => ['iframe_url' => $iframeUrl]];
    }

    public function callback(string $content, ?string $secureHash): array
    {
        Log::info('Callback received', ['request' => $content]);

        $data = json_decode($content, true);
        if (!$data) {
            Log::error('Invalid JSON payload', ['payload' => $content]);
            return ['status' => 400, 'payload' => ['error' => 'Invalid JSON payload']];
        }
        ...
        Log::info('Hash comparison', ['generated_hash' => $hashed, 'received_hash' => $secureHash]);

        if ($hashed !== $secureHash) {
            Log::warning('HMAC mismatch', ['status' => 'not secure']);
            return ['status' => 200, 'payload' => ['status' => 'not secure']];
        }
        ...
        if (!$localOrderId) {
            ...
            return ['status' => 400, 'payload' => ['status' => 'merchant_order_id missing']];
        }
        ...
        if (!$payment) {
            ...
            return ['status' => 404, 'payload' => ['status' => 'payment not found']];
        }
        ...
        return ['status' => 200, 'payload' => ['status' => 'callback handled']];
    }
